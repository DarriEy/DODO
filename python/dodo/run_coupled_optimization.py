"""
Coupled cFUSE + dRoute Differentiable Calibration (v3 - Enzyme AD)

This version properly couples:
- cFUSE: Using Enzyme AD via DifferentiableFUSEBatch
...
class CoupledFUSERoute(nn.Module):
    """
    Coupled cFUSE + dRoute model with end-to-end differentiability.
    ...
    def __init__(self,
        ...
        lowers = torch.tensor([cfuse.PARAM_BOUNDS[n][0] for n in self.param_names])
        uppers = torch.tensor([cfuse.PARAM_BOUNDS[n][1] for n in self.param_names])
        ...
        if routing_method == 'saint_venant':
            ...
            config.dt = dt  # 3600 seconds
            config.n_nodes = 10
            
            # Tighter tolerances OK for hourly (solver doesn't struggle as much)
            if dt <= 3600:  # Hourly or sub-hourly
                config.rel_tol = 1e-4
                config.abs_tol = 1e-6
                config.max_steps = 10000  # Much less than daily
                print(f"  Using standard tolerances for hourly timestep")
            else:  # Daily
                config.rel_tol = 1e-3
                config.abs_tol = 1e-4
                config.max_steps = 100000
                print(f"  Using relaxed tolerances for daily timestep")
            
            config.initial_depth = 0.5
            config.initial_velocity = 0.1
            config.min_depth = 0.01
            config.min_area = 0.1
            
            config.use_enzyme_jacobian = True
            config.use_enzyme_adjoint = True
            config.adjoint_checkpoint_steps = 100
    ...
    def _build_mapping_matrix(self, topology_file: str, hru_areas: np.ndarray) -> torch.Tensor:
        """Build HRU to reach mapping (converts mm/day to m³/s)."""
        ds = xr.open_dataset(topology_file)
        ...
        for h_idx, seg_id in enumerate(hru_to_seg):
            if seg_id in id_to_idx:
                ...
                # FUSE outputs mm/day regardless of timestep
                # mm/day * m² / 1000 / 86400 = m³/s
                conversion = hru_areas[h_idx] / 1000.0 / 86400.0  # Always 86400!
...
def train_model(model: CoupledFUSERoute,
                ...
                scheduler_type: str = 'warm_restarts',  # New parameter
    ...
    if scheduler_type == 'plateau':
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer, mode='min', factor=0.7, patience=30, min_lr=1e-6
        )
        ...
        scheduler = torch.optim.lr_scheduler.OneCycleLR(
            optimizer, max_lr=lr*3, total_steps=n_epochs,
...
def load_data_hourly(data_path: Path) -> Tuple[torch.Tensor, torch.Tensor, str, np.ndarray]:
    """Load hourly forcing data."""
    ...
    # FUSE forcing file - HOURLY version
    # Adjust path to your hourly input file
    ...
    precip = ds['pr'].values[:, :, 0]      # mm/hour (or mm/day - check your file!)
    temp = ds['temp'].values[:, :, 0]      # °C
    pet = ds['pet'].values[:, :, 0]        # mm/hour (or mm/day)
    q_obs = ds['q_obs'].values[:, :, 0]    # Check units!
    
    # IMPORTANT: Check if forcing is in mm/hour or mm/day
    # If your hourly file has mm/day rates, convert to mm/hour:
    # precip = precip / 24.0
    # pet = pet / 24.0
    ...
    print(f"  Time range: {n_timesteps} hours ({n_timesteps/24:.1f} days)")
    print(f"  HRUs: {n_hrus}")
    ...
    print(f"    Precip: mean={precip.mean():.4f}, max={precip.max():.4f} mm/hour")
    print(f"    PET: mean={pet.mean():.4f}, max={pet.max():.4f} mm/hour")
    print(f"    Temp: mean={temp.mean():.1f}, range=[{temp.min():.1f}, {temp.max():.1f}] °C")
    
    # Convert q_obs to m³/s
    # If q_obs is in mm/hour: mm/hour * m² / 1000 / 3600 = m³/s
    # If q_obs is already in m³/s, skip conversion
    total_area = hru_areas.sum()
    q_obs_outlet = q_obs[:, 0]
    
    # Adjust this based on your file's units!
    # If mm/hour over watershed:
    q_obs_cms = q_obs_outlet * total_area / 1000.0 / 3600.0
    # If already m³/s:
    # q_obs_cms = q_obs_outlet
    
    q_obs_cms[q_obs_outlet < 0] = float('nan')
...
def resolve_data_path(cli_path: Optional[str]) -> Path:
    """Resolve the dataset path for the distributed Bow-at-Banff example."""
    if cli_path:
        data_path = Path(cli_path).expanduser()
    else:
        env_path = os.environ.get("DODO_DATA_PATH")
        if env_path:
            data_path = Path(env_path).expanduser()
        else:
            data_path = Path.cwd() / "data" / "domain_Bow_at_Banff_distributed"
    if not data_path.exists():
        raise FileNotFoundError(
            "Example data not found. Provide --data-path or set DODO_DATA_PATH."
        )
    return data_path


def main():
    parser = argparse.ArgumentParser(description="Coupled cFUSE + dRoute optimization")
    parser.add_argument("--data-path", type=str, default=None,
                        help="Path to domain_Bow_at_Banff_distributed data")
    parser.add_argument("--output-dir", type=str, default="outputs",
                        help="Directory for outputs and plots")
    parser.add_argument("--epochs", type=int, default=500,
                        help="Number of training epochs")
    parser.add_argument("--routing-method", type=str, default="muskingum_cunge",
                        choices=[
                            "muskingum_cunge",
                            "diffusive_wave",
                            "diffusive_wave_ift",
                        ...
                        help="Routing method to use")
    parser.add_argument("--no-plots", action="store_true",
                        help="Disable plotting (useful for CI/headless)")
    parser.add_argument("--no-spatial-params", dest="spatial_params",
                        action="store_false",
                        help="Disable spatially varying FUSE parameters")
    parser.set_defaults(spatial_params=True)
    ...
    # Use hourly data loader
    forcing, observed, topo_file, hru_areas = load_data_hourly(DATA_PATH)
    ...
    # Forward-only methods (Manning's n is NOT optimized):
    #   'lag'                   - Simple lag routing
    #   'kwt'                   - Kinematic Wave Tracking (compatible)
    # --- Hourly timestep settings ---
    DT_SECONDS = 3600.0  # 1 hour in seconds
    DT_DAYS = 1.0 / 24.0  # For FUSE (expects days)
    WARMUP_HOURS = 30 * 24  # 30 days = 720 hours
    
    # Saint-Venant works MUCH better at hourly resolution!
    ROUTING_METHOD = args.routing_method
    
    print("\nInitializing model (HOURLY timesteps)...")
    model = CoupledFUSERoute(
        fuse_config=cfuse.VIC_CONFIG,
        topology_file=topo_file,
        hru_areas=hru_areas,
        dt=DT_SECONDS,           # 3600 seconds = 1 hour
        warmup_steps=WARMUP_HOURS,  # 720 hours = 30 days
        spatial_params=args.spatial_params,
    ...
    print("\nParameter bounds (from cfuse.PARAM_BOUNDS):")
    ...
    results = train_model(
        ...
        forcing=forcing,
        observed=observed,
        n_epochs=args.epochs,    # More epochs for complex loss
        lr=0.15,                # Learning rate
        loss_fn='kge',          # NSE + log-NSE + peak-weighted
        alpha=0.6,              # Not used for triple, but kept for reference
        spatial_reg=1.0,        # Slightly less regularization
        verbose=True
    )
    ...
    if not args.no_plots:
        ...
        print(f"  Saved routed vs unrouted to {OUTPUT_DIR / 'routed_vs_unrouted.png'}")
        plt.show()
